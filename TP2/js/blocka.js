...
;(function(){
    ...
    // Crea la path de un rectángulo redondeado (no hace fill/stroke)
    ...
    // Botón de ayuda (a la izquierda del botón Pausa)
    const helpW = 110, helpH = 32; const hx = px - helpW - 12; const hy = py;
    ctx.fillStyle = 'rgba(255,255,255,0.06)'; roundRectFill(ctx, hx, hy, helpW, helpH, 8);
    ctx.fillStyle = '#fff'; ctx.font = '600 14px Inter, sans-serif'; ctx.textAlign='center'; ctx.fillText('Ayuda +5s', hx + helpW/2, hy + helpH/2 + 1);
    gameState.hud.helpRect = { x: hx, y: hy, w: helpW, h: helpH };
    ...
    ctx.fillText('Movimientos: ' + (gameState.moves || 0), hud.hudX + 140, hud.hudY + hud.hudH/2);
        ...
        for (const p of gameState.pieces){
            ...
            // Dibujar con clipping redondeado (radio reducido para evitar distorsión)
            const radius = Math.min(12, Math.max(2, Math.floor(dw * 0.04)));
            ctx.beginPath();
            roundRectPath(ctx, -dw/2, -dh/2, dw, dh, radius);
            ctx.clip();
        ...
        if (gameState.isPaused){
            ...
            // Botón para volver al menú dentro del overlay de pausa
            const pbtnW = 220, pbtnH = 44;
            const pbtnX = (w - pbtnW) / 2;
            const pbtnY = h/2 + 48;
            ctx.fillStyle = styles.buttonBg;
            roundRectFill(ctx, pbtnX, pbtnY, pbtnW, pbtnH, 10);
            ctx.fillStyle = '#06202a'; ctx.font = '600 16px Inter, sans-serif'; ctx.fillText('Volver al menú', pbtnX + pbtnW/2, pbtnY + pbtnH/2 + 1);
            gameState.hud.pauseMenuBtn = { x: pbtnX, y: pbtnY, w: pbtnW, h: pbtnH };
        }
        ...
        if (gameState.win){
            ...
            // Botones: Siguiente nivel y Volver al menú
            ...
            ctx.fillStyle = '#06202a'; ctx.font = '600 16px Inter, sans-serif'; ctx.fillText('Siguiente nivel', bx1 + btnW/2, byBtn + btnH/2 + 1);
            // Volver al menú
            ...
            gameState.hud.winBtns = {
                next: { x: bx1, y: byBtn, w: btnW, h: btnH },
    ...
    function handleGamePointerDown(p, button){
        ...
        // Si el juego está pausado, permitir interacción con el botón de "Volver al menú" dentro del overlay
        if (gameState.isPaused) {
            const pm = gameState.hud && gameState.hud.pauseMenuBtn;
            if (pm && p.x >= pm.x && p.x <= pm.x + pm.w && p.y >= pm.y && p.y <= pm.y + pm.h){
                // Volver al menú desde pausa
                stopTimer();
                gameState.offCanvas = null;
        ...
        // Ayuda (si se clickea en HUD helpRect)
        const help = gameState.hud && gameState.hud.helpRect;
        if (help && p.x >= help.x && p.x <= help.x + help.w && p.y >= help.y && p.y <= help.y + help.h){
            // encontrar la primera pieza incorrecta
            const piece = gameState.pieces.find(pc => (pc.rotation % 360) !== 0);
            if (piece){
                piece.rotation = 0;
                // penalizar 5 segundos: mover startTime hacia atrás 5000ms (aumenta elapsed)
                if (gameState.startTime) gameState.startTime -= 5000;
                // comprobar victoria
                const won = gameState.pieces.every(p => (p.rotation % 360) === 0);
                if (won){ stopTimer(); gameState.win = true; console.info('Blocka: WIN detected (via help)'); }
            }
            render();
            return;
        }
        ...
        if (gameState.win && gameState.hud && gameState.hud.winBtns){
            ...
            // Siguiente nivel
            if (rb.next && p.x >= rb.next.x && p.x <= rb.next.x + rb.next.w && p.y >= rb.next.y && p.y <= rb.next.y + rb.next.h){
                // incrementar nivel y reiniciar (mismo blocks)
                gameState.hud.level = (gameState.hud.level || 1) + 1;
                gameState.moves = 0;
                gameState.elapsed = 0;
                gameState.win = false;
            ...
            if (rb.menu && p.x >= rb.menu.x && p.x <= rb.menu.x + rb.menu.w && p.y >= rb.menu.y && p.y <= rb.menu.y + rb.menu.h){
                // Volver al menú
